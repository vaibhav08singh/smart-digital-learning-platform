...
flowchart TB
    ...
    subgraph Server["⚙️ Server Layer (Next.js App Router)"]
        TutorRoute["/api/tutor"]
        AssistRoute["/api/ai/assist"]
        ExecuteRoute["/api/execute"]
        YouTubeRoute["/api/youtube"]
        EnvManager["🔐 Environment & Key Manager"]

        Services --> TutorRoute
        Services --> AssistRoute
        Services --> ExecuteRoute
        Services --> YouTubeRoute

        TutorRoute --> EnvManager
        AssistRoute --> EnvManager
        ExecuteRoute --> EnvManager
        YouTubeRoute --> EnvManager
    end

    subgraph External["🌐 External Services & APIs"]
        Gemini["✨ Google Gemini API"]
        OpenAI["🤖 OpenAI API"]
        Piston["💻 Piston Code Execution Backend"]
        YouTube["🎥 YouTube Data API v3"]

        EnvManager --> Gemini
        EnvManager --> OpenAI
        ExecuteRoute --> Piston
        YouTubeRoute --> YouTube
    end
```
